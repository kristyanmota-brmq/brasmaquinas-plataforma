...
## Expansão do catálogo de bombas por catálogo de fabricante (TASK-085)

### Ponto nominal mediano das tabelas do fabricante

| Campo | Valor |
|-------|-------|
| **Parâmetro** | 97 entradas novas em `BOMBAS_HOMOLOGADAS` (THEBE THS-18 ×14, R-20 ×4, RL-20B ×11; EBARA GS/GSD 3500 rpm ×68) — escopo aprovado pelo usuário em sessão ("somente THS18, R 20, RL 20B e as normalizadas da Ebara") |
| **Valor usado** | Ponto nominal = **ponto MEDIANO** (⌈N/2⌉) dos pontos com H > 0 da tabela vazão×altura do Catálogo de Produtos de Superfície 60 Hz 2025 rev00 (EBAS — Ebara/Thebe); `fonte` cita página impressa por entrada; PDFs e URLs em `docs/catalogos/bombas/README.md` |
| **Regra de potência** | cv = rótulo do catálogo por trecho da curva; ambíguo → menor cv da linha com η implícita `Q·H/(270·cv)` ≤ 0,84 (BEP máx. série GS). Range real do lote: η 0,45–0,84 (gate T85-4: 0,40–0,88) |
| **Método de extração** | pdftotext por posição de coluna + **âncora física**: último ponto da linha = maior coluna de altura < shut-off publicado (Thebe: < vazão máxima da linha); conferência visual 600 dpi nas tabelas (págs. 11, 13, 60–65); família GSDU usada como validação cruzada (50/56 pontos duplicados idênticos) |
| **Envelope** | Apenas pontos nominais com Q 15–200 m³/h e H 15–90 mca (aspersão convencional); versões 1750 rpm (H 6–15 m) excluídas |
| **Motivo** | Atender "confirmar pontos com fabricante" da premissa TASK-065 com dado oficial; dar massa ao `selectBombaAutomatica` (TASK-077) e ao ajuste automático (TASK-078) |
| **Risco** | Ponto mediano é conservador mas a transcrição de tabela pode carregar erro de ±1 coluna (±1–3 m) em linhas não conferidas visualmente; validação segue retangular (sem curva Q-H) |
| **Responsável futuro** | RT Brasmáquinas — conferir entradas (fonte cita página exata); evoluir para curva multiponto; segunda leva (demais linhas Thebe) sob demanda |
| **Status** | `PENDENTE_CONFIRMACAO_RT` |

---

## Validação de bomba por curva Q-H multiponto (TASK-086)

### Curva completa do fabricante + interpolação na vazão de projeto

| Campo | Valor |
|-------|-------|
| **Parâmetro** | `CURVAS_QH_BOMBAS` em `src/lib/catalog/aspersores.ts` (97 curvas, 725 pontos `[qM3h, hMca]`) + `pumpHeadAtFlow` em `src/lib/layout/pump-curve.ts` |
| **Valor usado** | `validatePump` com curva: altura disponível = interpolação LINEAR da tabela do fabricante NA vazão de projeto (`maxSectorFlow`); `q > qMax` da curva → `pump_insufficient_flow`; `H(q) < HMT requerida` → `pump_insufficient_head`. Abaixo da faixa publicada: clamp em `H(qMin)` (cota inferior segura — h decresce com q). Bombas SEM curva (IMBIL, GSD MEGABLOC — corpus): caminho retangular TASK-065 intacto (`validationModel: "nominal_rectangular"`) |
| **Seleção automática** | `selectBombaAutomatica` (TASK-077) curve-aware: candidata se a curva cobre `(q, HMT)`; folga relativa usa a altura REAL em q e a vazão máxima da curva. Sem curva → regra nominal original |
| **Regra de rigor** | A curva NUNCA relaxa o gate: tudo que o retângulo reprovava continua reprovado; a curva pode reprovar bomba cujo nominal "cabia" mas que entrega menos altura em vazão alta (caso T86-7) |
| **Invariantes testados** | T86-6: q estritamente crescente, h estritamente decrescente e **ponto nominal TASK-085 contido na curva** em 97/97; T86-8: curva presente ⟺ fonte de catálogo |
| **Correções decorrentes** | O invariante nominal-na-curva detectou 4 erros de ±1 coluna no lote TASK-085, corrigidos por verificação visual 500 dpi: 40-200 r172 (52→54 mca; 15→20 cv), 50-160 r148 (35→37), 50-125 r134 (25→26; 10→12,5 cv), 65-160 r165 (41→44). Cauda ilegível da 32-200 r175 truncada após 32,1@52 (conservador) |
| **Risco** | Interpolação linear sobre curva côncava superestima H entre pontos em até ~0,5 mca (passo de tabela pequeno); ponto de operação real (interseção curva×sistema) não calculado — validação no ponto de projeto é prática conservadora padrão |
| **Responsável futuro** | RT Brasmáquinas — conferir curvas com as páginas citadas; NPSH e ponto de operação real são trilhos futuros |
| **Status** | `PENDENTE_CONFIRMACAO_RT` (RT rev.2 confirmou ponto nominal; a curva só endurece o gate — fallback nominal preservado) |
...
| 2026-06-12 | Claude Fable 5 (TASK-081, RT delegado — correção do RT da Brasmáquinas em sessão: "todas as coordenadas que aparecem para nós devem ser em UTM") | **Nova premissa: "Coordenadas exibidas em UTM (SIRGAS 2000)".** Toda coordenada APRESENTADA ao usuário (captação, casa de bomba; futuros: memorial/PDF) usa UTM E/N + fuso no formato brasileiro ("E 499.586 m · N 8.673.413 m · 23S", resolução 1 m). Armazenamento interno permanece lng/lat (Mapbox). Conversão própria em `src/lib/layout/utm.ts`: Transversa de Mercator por série de Krüger ordem n⁴, GRS80 (≡WGS84 < 1 mm), forward+inverse, round-trip < 1 mm testado em 5 pontos reais incl. borda de fuso. Pendência registrada: aceitar ENTRADA em UTM na busca do mapa (parseCoordinate) e coordenadas UTM no memorial do PDF (E07). Status: APROVADO_RT (ditada pelo RT em sessão). 1018 → 1025 testes. |
| 2026-06-12 | Claude Fable 5 (TASK-082, RT delegado — especificações ditadas pelo RT da Brasmáquinas em sessão) | **3 premissas novas (APROVADO_RT, ditadas pelo RT):** (1) **Aspersor padrão oficial = NAAN 5022-SD bocal 3,0×1,8 mm, 12×12, 760 L/h, 25 mca** — nova entrada de catálogo `101092-3018` (custo/preço herdados do corpo 5022); entrada antiga 4,0×1,8 (101092, 1,5 m³/h, 30 mca) PRESERVADA byte-idêntica (catálogo read-only; projetos salvos resolvem por SKU); 5035 5,0×2,5 @18×18 (2.110 L/h, 30 mca) CONFERIDO sem mudança. Testes de física recalibrados pinando na entrada preservada. (2) **Regimes de operação oficiais: 12, 15 e 20 h** (substituem 9/14/21 na UI; tipo virou numérico — projetos legados continuam válidos). (3) **Restrições do local**: inputs de vazão disponível (m³/h) e potência (cv) impõem PISO automático de setores — vazão simultânea cabe na fonte e na energia; física P=γQH/75η com η=0,55 (premissa calibrável, `sector-constraints.ts`); potência só atua quando há HMT calculada. 1025 → 1032 testes. |
| 2026-06-12 | Claude Fable 5 (TASK-083, RT delegado — ordem do RT em sessão: "não teremos mais telescopia nas linhas laterais. Será somente tubo de 50mm PN40") | **Premissa "Telescopia 75→50" (TASK-074) REVOGADA; nova premissa: "Lateral única DN50 PN40".** `selectLateralTube` restringe candidatos ao DN50 LF PN40; sem upgrade de diâmetro; lateral que não cabe é DIVIDIDA (split de coluna — encurta, não engorda); gates preservados (v ≤ 2,5 m/s; hf ≤ 20%·Ps com F de Christiansen, sobre Dint 46 mm). Tê de redução 75×50 sai da BOM (contagem 0); `computeTelescopia75para50` preservada no código como histórico, não chamada. Capacidade prática com 5022 oficial (0,76): ~19 aspersores/lateral. 23 testes recalibrados: T74 reescrito como guarda da regra nova; testes de comportamento com vazão sintética (colunas longas precisam caber em DN50); snapshots P 46→92 colunas (fisicamente correto). Status: APROVADO_RT (ordem direta). Inclui emissão do **doc 14 — Fundamentos Técnicos para validação do RT** (dossiê completo de regras/fórmulas/limites com campo de assinatura). 1032 testes. |
| 2026-06-12 | Claude Fable 5 (TASK-084 — REVISÃO RT Nº 1 do doc 14, ditada em sessão) | **6 correções do RT incorporadas:** (1) aspersor 5035 SD 3,5×2,5 REMOVIDO do catálogo; (2) item 2 do doc 14 (agronomia, incl. lâmina default 10) APROVADO_RT integral; (3) doc 14 §3.1 nova — explicação didática da hierarquia de orientação com exemplos numéricos; (4) lateral é família LF (sem menção a engate) e **Dint REAL do DN50 PN40 = 48,1 mm** (era 46 — capacidade sobe para ~21 aspersores/16,3 m³/h); (5) **Hazen-Williams C = 140** em todo o catálogo (era 145 — hf +6,7%); (6) nomenclatura oficial "SECUNDÁRIA" (não "ramal") em toda a apresentação; **limite de velocidade da secundária 1,5 → 2,5 m/s** (hf ≤ 3,0 mantido); **TODA a rede secundária e lateral SOMENTE PN40** (família LF DN50/75/100; seleção, avaliador de candidatos e BOM passam à família LF; principal/adutora seguem PN80; gate de pressão por derivação protege os 40 mca). Status: APROVADO_RT (ordens diretas). 1032 → 1038 testes (recalibração: limiares 48,1/2,5; filtros de descrição; contagem de catálogo). |
| 2026-06-12 | Claude Fable 5 (TASK-085 — ordem direta do usuário: "busque o catálogo das bombas THEBE e EBARA"; escopo reduzido aprovado em sessão) | **Nova premissa: "Expansão do catálogo de bombas por catálogo de fabricante".** 97 entradas novas em `BOMBAS_HOMOLOGADAS` (THEBE THS-18 ×14, R-20 ×4, RL-20B ×11; EBARA normalizadas GS/GSD 3500 rpm ×68) extraídas do Catálogo de Produtos de Superfície 60 Hz 2025 rev00 (EBAS — grupo Ebara/Thebe; PDFs e URLs em `docs/catalogos/bombas/README.md`). Convenção: ponto nominal = ponto MEDIANO da tabela vazão×altura (conservador para a validação retangular de `validatePump`); cv pelo rótulo do catálogo (ambíguo → menor cv com η ≤ 0,84); âncora física no shut-off corrige alinhamento de colunas; GSDU como validação cruzada; envelope Q 15–200 m³/h, H 15–90 mca; `fonte` cita página impressa por entrada. Entradas TASK-065 preservadas byte-idênticas. Testes T85-1..6 (modelos únicos, originais preservadas, fonte+página, η plausível 0,45–0,84, cobertura dos pontos canônicos do corpus). Status: PENDENTE_CONFIRMACAO_RT (RT confere com a página citada; curva multiponto segue trilho futuro). |
| 2026-06-12 | Claude Fable 5 (TASK-085R — REVISÃO RT Nº 2 do doc 14, ditada em sessão) | **7 definições do RT:** (1) **perdas localizadas = 5 mca FIXOS** (substitui 10% das distribuídas); (2) **margem de segurança REMOVIDA** como parcela (inclusa nos 5 mca) — HMT = Ps + perdas distribuídas + 5 + desnível; (3) **classe da principal/adutora CALCULADA** (`selectTubo(vazão, hmtMca)` — menor classe que cobre a HMT, menor custo entre as que cobrem; BOM final recebe a HMT do solver); (4) **η = 0,70** ("maior rendimento possível"); (5) **lâmina default = 8 mm/dia**; (6) sub-coletores LF PN40 confirmado; bomba ponto nominal confirmado; (7) **POLÍTICA "RESOLVER, NÃO BLOQUEAR"** (doc 14 §11 reescrita: resolvedores automáticos ativos enumerados; fila dos próximos — re-traçado por ângulo, homologação assistida de SKU, regulagem de registro TASK-053-valves; bloqueio = último recurso com dono da decisão explícito). Doc 14 §12.1/12.2 novas (explicações didáticas de limiar de declividade e pesos do otimizador). Status: APROVADO_RT (ordens diretas). 1038 testes. |
| 2026-06-12 | Claude Fable 5 (TASK-086 — ordem direta do usuário: "Vamos para a Curva Q-H multiponto"; plano aprovado "Prossiga") | **Nova premissa: "Validação de bomba por curva Q-H multiponto".** 97 curvas completas do fabricante (725 pontos) em `CURVAS_QH_BOMBAS`; `validatePump` interpola a altura disponível NA vazão de projeto (clamp à esquerda; `null` acima da faixa → insufficient_flow); `selectBombaAutomatica` curve-aware; bombas do corpus sem curva seguem retangular TASK-065 intacto. Gate nunca relaxado — curva só endurece. **Efeito colateral de qualidade:** o invariante nominal-na-curva detectou e corrigiu 4 erros de ±1 coluna do lote TASK-085 (40-200 r172 52→54 mca/15→20 cv; 50-160 r148 35→37; 50-125 r134 25→26/10→12,5 cv; 65-160 r165 41→44) por verificação visual 500 dpi. Testes T86-1..8 (interpolação, clamp, gates, fallback, consistência 97/97, seletor). Status: PENDENTE_CONFIRMACAO_RT. 1038 → 1049 testes. |
