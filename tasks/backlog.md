...
Testes na base: 870/870 · TypeScript: 0 erros · 27/27 testes tooling · Working tree: modificado (TASK-053 — espinha de peixe SEMPRE sub-coletor v12) — **TASK-052 publicada em `origin/main` (commit `ca5bdd9`) + TASK-053 v12 entrega topologia "espinha de peixe operacional" SEMPRE sub-coletor: TODA lateral conecta via `rib → spine → spine_entry → principal` (nenhuma conexão direta lateral→principal quando `operationalSegments` é fornecido). 3 entidades lineares por setor: 1 `spine` perpendicular aos laterais (eixo X do frame rotacionado por `gridAngleDegrees`, posicionado no headland entre principal e inlets via midpoint `(principalYLocal + farthestInletYLocal) / 2`; fallback `MIN_HEADLAND_M = 3 m`) + 1 `spine_entry` perpendicular conectando principal ao spine + N `ribs` perpendiculares ao spine (1 por coluna). `fieldSideSign` via centroid LngLat (v12 fix MET-053-V11-01 — independente do range dos inlets; evita `Math.sign(0) === 0` colapsar fallback). Gate explícito `throw` (v12 fix TECH-053-V11-02) quando `operationalSegments` sem `gridAngleDegrees`. Mudança em `hydraulic-connectivity.ts` (tipo `SecondaryPipe` com `kind: "spine" \| "spine_entry" \| "rib"` + `routeEspinhaDePeixe`), `secondary-sizing.ts` (3 paths kind-aware: Path 0 legado preservado byte-a-byte; Path 1 ribs com `max(coluna)`; Path 2 spine/spine_entry com SUM ribs do sectorId), `network-angle-diagnostics.ts` (validação kind-aware: legado completo; spine_entry só junção→principal; rib só junção→lateral; spine skip sob garantia construtiva), `irrigation-project.ts` (call site), `map-consistency.ts`. Histórico v1..v12 (5 reprovações arquiteturais + 5 iterações documentais): v1 espigão 180°, v2 INV-LAYOUT violada terminal, v3 stair-step (falhou visualmente em grid rotacionado 59°), v4 "T deitado" ambíguo, v5 omissão path legado, v6 aprovado funcional + reprovado visual em Projeto A (degenerescência: probe central de `principalYLocal` coincide com principal quando inlets na borda), v7 topologia INVERTIDA (spine paralelo à principal), v8 heurística X-vs-Y reprovada, v9 diagnóstico-only, v10 cohorts (mediana de gaps com zeros = 0), v11 `Math.sign(0)` colapsa fallback, **v12 aprovado_com_ajustes via OVERRIDE técnico Caminho 2** (2 blockers metodológicos não-terminais endereçados como compromissos). SEM mexer em `bom.ts` (RB-05 + INV-LAYOUT-INSTAVEL-COMERCIAL); SEM ADR formal (ADR-016 fica para TASK-054 sucessora APÓS validação visual RT); SEM `constructability.ts` (section_valve relocation para spine_entry DEFERIDA para TASK-053-valves). **Blocker TECH-053-01 permanece ATIVO ao fechar** (11 blockers rib→lateral geometricamente inevitáveis com regra estrita `[0°, 90°]` no Projeto A) — fechamento técnico (tsc 0 + vitest 870/870 + tooling 27/27) **NÃO é fechamento comercial**; emissão de proposta comercial bloqueada por default até decisão RT explícita (override técnico OU aguardar TASK-053-valves para mitigar via section_valve no spine_entry). Validação visual no Projeto A continua sendo gatekeeper de homologação**
