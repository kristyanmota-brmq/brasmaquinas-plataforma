...
**Emitido em:** 2026-06-12 · **Para:** RT da Brasmáquinas · **Status da base:** 1032 testes automatizados, 0 erros de tipo
...
Entradas adicionais (5035 3,5×2,5; 5035-PC 4,5; 5022 4,0×1,8 preservada) disponíveis por seleção explícita. Catálogo é *read-only*: especificação nunca muda sob um SKU existente; projetos salvos resolvem pelo SKU gravado. `src/lib/catalog/aspersores.ts`.
...
- **Setores derivados:** `n = floor(horas disponíveis / t)` — equação extraída das propostas reais da casa. Disponível como modo "agronômico" (toggle); modo legado `setores = jornada` preservado. APROVADO_RT (como diagnóstico).
...
- **Tubo ÚNICO: DN50 PN40 (LF, engate)** — sem telescopia (revoga a cascata 75→50), sem upgrade de diâmetro. Lateral que não cabe **divide** (encurta, não engorda) — split automático de coluna no menor número necessário.
- **Gates da lateral (sobre diâmetro INTERNO, 46 mm):** velocidade ≤ **2,5 m/s**; perda de carga (com fator de Christiansen `F(n)` para saída múltipla) ≤ **20% da pressão de serviço** do aspersor. Capacidade prática 5022 novo (0,76): **até ~19 aspersores por lateral** pela velocidade; comprimento limitado pela perda.
...
- **Hazen-Williams**, C = 145 (PVC), sempre com **diâmetro interno real** de catálogo: `hf = 10,67 · L · (Q/C)^1,852 / D^4,87`.
- Laterais: F de Christiansen; caminho crítico **exaustivo** (todos os setores × todos os trechos).
- **Limites:** ramal/sub-coletor v ≤ 1,5 m/s (NRCS) e hf ≤ 3,0 mca (10%×30) — APROVADO_RT; lateral v ≤ 2,5 m/s e hf ≤ 20%·Ps; classe de pressão (PN) conferida POR TRECHO com pressão real por derivação (`HMT − hf acumulado`) — violação confirmada é blocker.
- **Perdas localizadas: +10%** das distribuídas (CALIBRÁVEL). **HMT = pressão de serviço + Σ perdas + desnível geodético + margem de segurança**.
- Tubos: laterais LF PN40 DN50 (único); sub-coletores rígido PN80/DEFOFO PN60 (PN60 só onde pressão ≤ 60); principal/adutora PN80. `hydraulic-sizing.ts`, `hazenWilliams.ts`.
